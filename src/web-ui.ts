...
interface OpsMissionAction {
  type: OpsActionQueueItem['type'];
  payload: JsonObject;
}
...
interface OpsMission {
  ...
  preflightUrl?: string;
  gatePolicy?: JsonObject;
  queuePlan: OpsMissionAction[];
  autoRunQueue: boolean;
}

interface OpsMissionRun {
  id: number;
  mission: string;
  startedAt: string;
  finishedAt: string;
...
let opsMissions: Record<string, OpsMission> = loadOpsMissions();
...
let opsMissionPolicyState: OpsMissionPolicyState = loadOpsMissionPolicyState();
...
function isOpsActionType(value: string): value is OpsActionQueueItem['type'] {
  return (
    value === 'stabilize' ||
    value === 'watchdog' ||
    value === 'chaos-drill' ||
...
function defaultOpsMissions(): Record<string, OpsMission> {
  return {
    'release-guard-default': {
      name: 'release-guard-default',
      updatedAt: nowIso(),
      description: 'Default release mission for stabilization + guard checks.',
      scenarioName: 'stability-default',
      policyPresetName: 'balanced-default',
      watchdogProfileName: 'watchdog-default',
      preflightUrl: 'https://www.wikipedia.org',
      gatePolicy: {
        minScore: 80,
        allowedSeverity: 'green',
        maxFailedJobs: 0,
...
function normalizeMissionQueuePlan(value: unknown): OpsMissionAction[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const normalized: OpsMissionAction[] = [];
...
function loadOpsMissions(): Record<string, OpsMission> {
  try {
    ensureAppStateDir();
    if (!fs.existsSync(OPS_MISSIONS_FILE)) {
      const defaults = defaultOpsMissions();
      fs.writeFileSync(OPS_MISSIONS_FILE, JSON.stringify(defaults, null, 2), 'utf8');
      return defaults;
    }
    const raw = fs.readFileSync(OPS_MISSIONS_FILE, 'utf8');
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      return defaultOpsMissions();
    }
    const result: Record<string, OpsMission> = {};
    for (const [key, value] of Object.entries(parsed as Record<string, unknown>)) {
      if (!value || typeof value !== 'object' || Array.isArray(value)) {
        continue;
      }
      ...
      const gatePolicyRaw = item.gatePolicy && typeof item.gatePolicy === 'object' && !Array.isArray(item.gatePolicy)
        ? (item.gatePolicy as JsonObject)
        : undefined;
      const preflightUrl = typeof item.preflightUrl === 'string' && /^https?:\/\//i.test(item.preflightUrl)
        ? item.preflightUrl
        : undefined;
      result[name] = {
        name,
        updatedAt: typeof item.updatedAt === 'string' ? item.updatedAt : nowIso(),
        description: typeof item.description === 'string' ? item.description : undefined,
        scenarioName: typeof item.scenarioName === 'string' && item.scenarioName.trim().length > 0 ? item.scenarioName.trim() : undefined,
        policyPresetName: typeof item.policyPresetName === 'string' && item.policyPresetName.trim().length > 0 ? item.policyPresetName.trim() : undefined,
        watchdogProfileName: typeof item.watchdogProfileName === 'string' && item.watchdogProfileName.trim().length > 0 ? item.watchdogProfileName.trim() : undefined,
        preflightUrl,
        gatePolicy: gatePolicyRaw ? normalizeOpsGatePolicy(gatePolicyRaw) : undefined,
        queuePlan: normalizeMissionQueuePlan(item.queuePlan),
        autoRunQueue: item.autoRunQueue !== false,
      };
    }
    return Object.keys(result).length > 0 ? result : defaultOpsMissions();
  } catch {
    return defaultOpsMissions();
  }
}

function saveOpsMissions(): void {
  ensureAppStateDir();
  fs.writeFileSync(OPS_MISSIONS_FILE, JSON.stringify(opsMissions, null, 2), 'utf8');
}
...
function defaultOpsMissionPolicyState(): OpsMissionPolicyState {
  return {
    blockSchedulesOnGateFail: true,
...
function loadOpsMissionPolicyState(): OpsMissionPolicyState {
  try {
    ensureAppStateDir();
    if (!fs.existsSync(OPS_MISSION_POLICY_FILE)) {
      const defaults = defaultOpsMissionPolicyState();
      fs.writeFileSync(OPS_MISSION_POLICY_FILE, JSON.stringify(defaults, null, 2), 'utf8');
      return defaults;
    }
    const raw = fs.readFileSync(OPS_MISSION_POLICY_FILE, 'utf8');
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      return defaultOpsMissionPolicyState();
    }
    const item = parsed as Record<string, unknown>;
    const result: OpsMissionPolicyState = {
      blockSchedulesOnGateFail: item.blockSchedulesOnGateFail !== false,
      maxConsecutiveFailures: clampInt(item.maxConsecutiveFailures, 3, 1, 1000),
      cooldownMs: clampInt(item.cooldownMs, 180000, 5000, 24 * 60 * 60 * 1000),
      autoResumeAfterCooldown: item.autoResumeAfterCooldown !== false,
      suspended: item.suspended === true,
      suspendedAt: typeof item.suspendedAt === 'string' ? item.suspendedAt : undefined,
      suspendedReason: typeof item.suspendedReason === 'string' ? item.suspendedReason : undefined,
      resumeAt: typeof item.resumeAt === 'string' ? item.resumeAt : undefined,
      consecutiveFailures: clampInt(item.consecutiveFailures, 0, 0, 1000000),
      updatedAt: typeof item.updatedAt === 'string' ? item.updatedAt : nowIso(),
    };
    return result;
  } catch {
...
function saveOpsMissionPolicyState(): void {
  ensureAppStateDir();
  fs.writeFileSync(OPS_MISSION_POLICY_FILE, JSON.stringify(opsMissionPolicyState, null, 2), 'utf8');
}

function maybeResumeOpsMissionPolicyState(): void {
  if (!opsMissionPolicyState.suspended) {
    return;
  }
  if (!opsMissionPolicyState.autoResumeAfterCooldown) {
    return;
  }
  const resumeAtMs = opsMissionPolicyState.resumeAt ? Date.parse(opsMissionPolicyState.resumeAt) : Number.NaN;
  ...
  opsMissionPolicyState.suspended = false;
  opsMissionPolicyState.suspendedAt = undefined;
  opsMissionPolicyState.suspendedReason = undefined;
  opsMissionPolicyState.resumeAt = undefined;
  opsMissionPolicyState.consecutiveFailures = 0;
  opsMissionPolicyState.updatedAt = nowIso();
  saveOpsMissionPolicyState();
  pushEvent('ops-mission-policy-auto-resume', 'Mission scheduler auto-resumed after cooldown', {});
}

function currentOpsMissionPolicyState(): OpsMissionPolicyState {
  maybeResumeOpsMissionPolicyState();
  return {
    ...opsMissionPolicyState,
  };
}

function suspendOpsMissionPolicyState(reason: string): OpsMissionPolicyState {
  const state = currentOpsMissionPolicyState();
  state.suspended = true;
  state.suspendedAt = nowIso();
  state.suspendedReason = reason;
  state.resumeAt = new Date(Date.now() + state.cooldownMs).toISOString();
  state.updatedAt = nowIso();
  opsMissionPolicyState = state;
  saveOpsMissionPolicyState();
  pushEvent('ops-mission-policy-suspended', 'Mission scheduler suspended', {
    reason,
    resumeAt: state.resumeAt,
  });
  return currentOpsMissionPolicyState();
}

function resumeOpsMissionPolicyState(reason = 'manual-resume'): OpsMissionPolicyState {
  const state = currentOpsMissionPolicyState();
  ...
  opsMissionPolicyState = state;
  saveOpsMissionPolicyState();
  pushEvent('ops-mission-policy-resumed', 'Mission scheduler resumed', { reason });
  return currentOpsMissionPolicyState();
}

function updateOpsMissionPolicyState(body: JsonObject): OpsMissionPolicyState {
  const current = currentOpsMissionPolicyState();
  opsMissionPolicyState = {
    ...current,
    blockSchedulesOnGateFail: body.blockSchedulesOnGateFail === undefined ? current.blockSchedulesOnGateFail : body.blockSchedulesOnGateFail !== false,
    maxConsecutiveFailures: clampInt(body.maxConsecutiveFailures, current.maxConsecutiveFailures, 1, 1000),
    cooldownMs: clampInt(body.cooldownMs, current.cooldownMs, 5000, 24 * 60 * 60 * 1000),
    autoResumeAfterCooldown: body.autoResumeAfterCooldown === undefined ? current.autoResumeAfterCooldown : body.autoResumeAfterCooldown !== false,
    updatedAt: nowIso(),
  };
  saveOpsMissionPolicyState();
  pushEvent('ops-mission-policy-updated', 'Mission scheduler policy updated', {
    blockSchedulesOnGateFail: opsMissionPolicyState.blockSchedulesOnGateFail,
    maxConsecutiveFailures: opsMissionPolicyState.maxConsecutiveFailures,
    cooldownMs: opsMissionPolicyState.cooldownMs,
    autoResumeAfterCooldown: opsMissionPolicyState.autoResumeAfterCooldown,
  });
  return currentOpsMissionPolicyState();
}

function missionSchedulePolicyForecast(horizonMsRaw: unknown): JsonObject {
  const state = currentOpsMissionPolicyState();
  ...
  for (const schedule of listOpsMissionSchedules()) {
    if (!schedule.active) {
      continue;
    }
    const everyMs = Math.max(5000, schedule.everyMs);
    const startMs = schedule.lastRunAt ? Date.parse(schedule.lastRunAt) : now;
    let pointer = Number.isFinite(startMs) ? startMs + everyMs : now + everyMs;
    let count = 0;
    while (pointer <= until && count < 100) {
      items.push({
        id: schedule.id,
        name: schedule.name,
        mission: schedule.mission,
        at: new Date(pointer).toISOString(),
      });
      pointer += everyMs;
      count += 1;
    }
...
function planRunbookCampaign(body: JsonObject): JsonObject {
  const runbookName = typeof body.name === 'string' ? body.name.trim() : '';
  if (!runbookName) {
    throw new Error('runbook name is required');
  }
  const requestedDeviceIds = Array.isArray(body.deviceIds)
    ? body.deviceIds.filter((value): value is string => typeof value === 'string' && value.trim().length > 0)
    : [];
  const connectedDeviceIds = getConnectedDevices().map(device => device.id);
...
function runWatchdog(body: JsonObject, host: string, port: number): JsonObject {
  const controlBefore = buildControlRoomPayload(host, port);
  const diagnostics = buildDiagnosticsReport(host, port);
  const alertsCheck = evaluateAlertRulesNow();
  const autoHealOnAlert = body.autoHealOnAlert === true;
  let autoHealResult: JsonObject | undefined;
...
function normalizeOpsGatePolicy(input: JsonObject): JsonObject {
  return {
    minScore: clampInt(input.minScore, 80, 0, 100),
    allowedSeverity: typeof input.allowedSeverity === 'string' ? input.allowedSeverity : 'green',
    maxFailedJobs: clampInt(input.maxFailedJobs, 0, 0, 100000),
    maxOpenIncidents: clampInt(input.maxOpenIncidents, 0, 0, 100000),
  };
}

function evaluateOpsGateWithPolicy(policyInput: JsonObject, host: string, port: number): JsonObject {
  const policy = normalizeOpsGatePolicy(policyInput);
  const control = buildControlRoomPayload(host, port);
  const diagnostics = buildDiagnosticsReport(host, port);
...
function currentOpsGatePolicy(): JsonObject {
  return normalizeOpsGatePolicy(opsGatePolicy);
}

function updateOpsGatePolicy(body: JsonObject): JsonObject {
  const current = currentOpsGatePolicy();
  opsGatePolicy = normalizeOpsGatePolicy({
    minScore: body.minScore ?? current.minScore,
    allowedSeverity: typeof body.allowedSeverity === 'string' ? body.allowedSeverity : current.allowedSeverity,
    maxFailedJobs: body.maxFailedJobs ?? current.maxFailedJobs,
    maxOpenIncidents: body.maxOpenIncidents ?? current.maxOpenIncidents,
  });
...
function evaluateOpsGate(host: string, port: number): JsonObject {
  const result = evaluateOpsGateWithPolicy(currentOpsGatePolicy(), host, port);
  return {
    ...result,
    checkedAt: nowIso(),
    updateHint: UPDATE_HINT,
...
function saveOpsMission(nameRaw: string, body: JsonObject): OpsMission {
  const name = nameRaw.trim();
  if (!name) {
    throw new Error('mission name is required');
  }
  const queuePlan = normalizeMissionQueuePlan(body.queuePlan);
  const mission: OpsMission = {
    name,
    updatedAt: nowIso(),
    description: typeof body.description === 'string' ? body.description : undefined,
    scenarioName: typeof body.scenarioName === 'string' && body.scenarioName.trim().length > 0 ? body.scenarioName.trim() : undefined,
    policyPresetName: typeof body.policyPresetName === 'string' && body.policyPresetName.trim().length > 0 ? body.policyPresetName.trim() : undefined,
    watchdogProfileName: typeof body.watchdogProfileName === 'string' && body.watchdogProfileName.trim().length > 0 ? body.watchdogProfileName.trim() : undefined,
    preflightUrl: typeof body.preflightUrl === 'string' && /^https?:\/\//i.test(body.preflightUrl) ? body.preflightUrl : undefined,
    gatePolicy: body.gatePolicy && typeof body.gatePolicy === 'object' && !Array.isArray(body.gatePolicy)
      ? normalizeOpsGatePolicy(body.gatePolicy as JsonObject)
      : undefined,
    queuePlan,
    autoRunQueue: body.autoRunQueue !== false,
  };
  if (mission.queuePlan.length === 0 && mission.scenarioName) {
    mission.queuePlan.push({
      type: 'scenario-run',
      payload: { name: mission.scenarioName },
    });
  }
  opsMissions[name] = mission;
  saveOpsMissions();
  pushEvent('ops-mission-saved', 'Ops mission saved', { name });
  return mission;
}

function deleteOpsMission(nameRaw: string): boolean {
  const name = nameRaw.trim();
  if (!name || !opsMissions[name]) {
    return false;
  }
  delete opsMissions[name];
  saveOpsMissions();
  pushEvent('ops-mission-deleted', 'Ops mission deleted', { name });
  return true;
}
...
function runOpsMissionScheduleTask(id: number): JsonObject {
  ...
  const policyBefore = currentOpsMissionPolicyState();
  if (policyBefore.suspended) {
    return {
      ok: false,
      id,
      skipped: true,
      reason: 'policy-suspended',
      resumeAt: policyBefore.resumeAt,
    };
  }
  ...
  try {
    ...
    let nextPolicy = currentOpsMissionPolicyState();
    const gateRaw = result.gate && typeof result.gate === 'object' && !Array.isArray(result.gate)
      ? (result.gate as Record<string, unknown>)
      : undefined;
    const gatePass = gateRaw?.pass !== false;
    if (result.ok === true && gatePass) {
      if (nextPolicy.consecutiveFailures !== 0) {
        nextPolicy.consecutiveFailures = 0;
        nextPolicy.updatedAt = nowIso();
        opsMissionPolicyState = nextPolicy;
        saveOpsMissionPolicyState();
      }
    } else {
      nextPolicy.consecutiveFailures += 1;
      nextPolicy.updatedAt = nowIso();
      const shouldSuspend = nextPolicy.blockSchedulesOnGateFail &&
        nextPolicy.consecutiveFailures >= nextPolicy.maxConsecutiveFailures;
      opsMissionPolicyState = nextPolicy;
      saveOpsMissionPolicyState();
      if (shouldSuspend) {
        nextPolicy = suspendOpsMissionPolicyState(`schedule-failure-threshold(${nextPolicy.consecutiveFailures})`);
      }
    }
    ...
    schedule.updatedAt = nowIso();
    const nextPolicy = currentOpsMissionPolicyState();
    nextPolicy.consecutiveFailures += 1;
    nextPolicy.updatedAt = nowIso();
    opsMissionPolicyState = nextPolicy;
    saveOpsMissionPolicyState();
    if (nextPolicy.blockSchedulesOnGateFail &&
      nextPolicy.consecutiveFailures >= nextPolicy.maxConsecutiveFailures) {
      suspendOpsMissionPolicyState(`schedule-exception-threshold(${nextPolicy.consecutiveFailures})`);
    }
    saveOpsMissionSchedules();
    ...
    return {
      ...
      mission: schedule.mission,
      error: message,
      policy: currentOpsMissionPolicyState(),
    };
  } finally {
...
function setAllOpsMissionSchedulesActive(active: boolean): JsonObject {
  ...
  for (const schedule of Object.values(opsMissionSchedules)) {
    if (active) {
      ...
      stopOpsMissionSchedule(schedule.id);
      changed.push(schedule.id);
    }
  }
  return {
...
function rebalanceOpsMissionSchedules(body: JsonObject): JsonObject {
  const factorRaw = Number(body.factor);
  const factor = Number.isFinite(factorRaw) ? Math.max(0.1, Math.min(10, factorRaw)) : 1;
  const minEveryMs = clampInt(body.minEveryMs, 5000, 5000, 24 * 60 * 60 * 1000);
  const maxEveryMs = clampInt(body.maxEveryMs, 24 * 60 * 60 * 1000, 5000, 24 * 60 * 60 * 1000);
  const touched: number[] = [];

  for (const schedule of Object.values(opsMissionSchedules)) {
    const before = schedule.everyMs;
    const scaled = Math.trunc(before * factor);
    schedule.everyMs = Math.max(minEveryMs, Math.min(maxEveryMs, scaled));
    schedule.updatedAt = nowIso();
    touched.push(schedule.id);
    if (schedule.active) {
      startOpsMissionSchedule(schedule.id);
    }
  }
  saveOpsMissionSchedules();
  pushEvent('ops-mission-schedules-rebalanced', 'Ops mission schedules rebalanced', {
    factor,
...
function resolveOpsMission(nameRaw: string): OpsMission {
  ...
  const mission = opsMissions[name];
  if (!mission) {
...
function planOpsMission(nameRaw: string, body: JsonObject, host: string, port: number): JsonObject {
  const mission = resolveOpsMission(nameRaw);
  const scenarioExists = mission.scenarioName ? Boolean(opsScenarios[mission.scenarioName]) : true;
  const policyPresetExists = mission.policyPresetName ? Boolean(opsPolicyPresets[mission.policyPresetName]) : true;
  const watchdogProfileExists = mission.watchdogProfileName ? Boolean(watchdogProfiles[mission.watchdogProfileName]) : true;
  const unknownActionTypes = mission.queuePlan
    .map(item => item.type)
    .filter(type => !isOpsActionType(type));
  const gatePreview = evaluateOpsGateWithPolicy(
    mission.gatePolicy ? mission.gatePolicy : currentOpsGatePolicy(),
    host,
    port
  );
  const checks = [
    {
      check: 'scenario',
      pass: scenarioExists,
      value: mission.scenarioName || '-',
    },
    {
      check: 'policyPreset',
      pass: policyPresetExists,
      value: mission.policyPresetName || '-',
    },
    {
      check: 'watchdogProfile',
      pass: watchdogProfileExists,
      value: mission.watchdogProfileName || '-',
    },
    {
      check: 'queuePlanTypes',
      pass: unknownActionTypes.length === 0,
      value: unknownActionTypes.length === 0 ? 'ok' : unknownActionTypes.join(','),
    },
    {
      check: 'gatePreview',
      pass: gatePreview.pass === true,
      value: gatePreview.pass === true ? 'pass' : 'fail',
    },
  ];
  ...
  return {
    ...
    checks,
    gatePreview,
    suggestedMaxQueueRun: Math.max(1, mission.queuePlan.length || 1),
    updateHint: UPDATE_HINT,
  };
}
...
function runOpsMission(nameRaw: string, body: JsonObject, host: string, port: number): JsonObject {
  ...
  const steps: JsonObject[] = [];
  let gate: JsonObject | undefined;
  const laneId = typeof body.laneId === 'string' && body.laneId.trim().length > 0 ? body.laneId.trim() : undefined;

  try {
    if (mission.policyPresetName) {
      const appliedPolicy = applyOpsPolicyPreset(mission.policyPresetName);
      steps.push({
        step: 'apply-policy-preset',
        ok: true,
        name: mission.policyPresetName,
        policy: appliedPolicy.policy,
      });
    }

    if (mission.gatePolicy) {
      const updatedGate = updateOpsGatePolicy(mission.gatePolicy);
      steps.push({
        step: 'apply-gate-policy',
        ok: true,
        policy: updatedGate,
      });
    }
    ...
    const url = typeof body.url === 'string' && /^https?:\/\//i.test(body.url)
      ? body.url
      : mission.preflightUrl;
    if (url) {
      const openResult = openUrlInChrome(url, extractDeviceId(body), {
        waitForReadyMs: clampInt(body.waitForReadyMs, 900, 200, 15000),
        fallbackToDefault: true,
      });
      steps.push({
        step: 'preflight-open-url',
        ok: true,
        url,
        deviceId: openResult.deviceId,
      });
    ...
    if (mission.scenarioName) {
      const scenarioResult = runOpsScenario({
        ...body,
        name: mission.scenarioName,
        laneId,
      }, host, port);
      steps.push({
        step: 'scenario-run',
        ok: scenarioResult.ok !== false,
        name: mission.scenarioName,
      });
    }

    if (mission.watchdogProfileName) {
      const watchdogResult = runWatchdogProfile(mission.watchdogProfileName, {
        ...body,
        laneId,
      }, host, port);
      steps.push({
        step: 'watchdog-profile-run',
        ok: watchdogResult.ok !== false,
        name: mission.watchdogProfileName,
      });
    }

    const queued: OpsActionQueueItem[] = [];
    for (const action of mission.queuePlan) {
      const payload: JsonObject = action.payload && typeof action.payload === 'object' ? { ...action.payload } : {};
      if (laneId && typeof payload.laneId !== 'string') {
        payload.laneId = laneId;
      }
      if (action.type === 'scenario-run' && mission.scenarioName && typeof payload.name !== 'string') {
        payload.name = mission.scenarioName;
      }
      const item = enqueueOpsAction({
        type: action.type,
        payload,
      });
      queued.push(item);
    ...
    steps.push({
      step: 'enqueue-actions',
      ok: true,
      queued: queued.length,
    });

    if ((mission.autoRunQueue || body.autoRunQueue === true) && queued.length > 0) {
      const maxItems = clampInt(body.maxQueueItems, queued.length, 1, 400);
      const queueRun = runAllOpsActions(host, port, maxItems);
      steps.push({
        step: 'run-action-queue',
        ok: queueRun.ok !== false,
        executed: queueRun.executed,
      });
    }

    gate = evaluateOpsGateWithPolicy(
      mission.gatePolicy ? mission.gatePolicy : currentOpsGatePolicy(),
      host,
      port
    );
    const ok = steps.every(item => item.ok !== false) && gate.pass === true;
    const finishedAt = nowIso();
    const durationMs = Date.now() - startedMs;

    const record: OpsMissionRun = {
      id: opsMissionRunSeq++,
...
function diffQueueSnapshot(nameRaw: string): JsonObject {
  const name = nameRaw.trim();
  if (!name) {
    throw new Error('snapshot name is required');
  }
  ...
  const laneDiffs: Array<{
    laneId: string;
    snapshotQueued: number;
    currentQueued: number;
    delta: number;
  }> = [];
  const laneIds = new Set<string>([...storedMap.keys(), ...currentMap.keys()]);
...
async function handleApi(
  ...
  if (method === 'GET' && pathname === '/api/ops/missions') {
    await withMetric('ops-missions-list', () => {
      sendJson(response, 200, {
        ok: true,
  ...
  if (method === 'POST' && pathname === '/api/ops/missions') {
    await withMetric('ops-missions-save', async () => {
      const body = await readJsonBody(request);
      const name = typeof body.name === 'string' ? body.name : '';
      try {
        const mission = saveOpsMission(name, body);
        sendJson(response, 200, { ok: true, mission, updateHint: UPDATE_HINT });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        sendJson(response, 400, { error: message });
      }
  ...
  if (method === 'POST' && pathname === '/api/ops/missions/plan') {
    await withMetric('ops-missions-plan', async () => {
      const body = await readJsonBody(request);
      const name = typeof body.name === 'string' ? body.name : '';
      try {
        sendJson(response, 200, planOpsMission(name, body, context.host, context.port));
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        sendJson(response, 400, { error: message });
      }
  ...
  if (method === 'POST' && pathname === '/api/ops/missions/run') {
    await withMetric('ops-missions-run', async () => {
      const body = await readJsonBody(request);
      const name = typeof body.name === 'string' ? body.name : '';
      try {
        sendJson(response, 200, runOpsMission(name, body, context.host, context.port));
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        sendJson(response, 400, { error: message });
      }
  ...
  if (method === 'GET' && pathname === '/api/ops/missions/runs') {
    await withMetric('ops-missions-runs-list', () => {
      const limitRaw = Number(url.searchParams.get('limit') ?? '120');
      sendJson(response, 200, listOpsMissionRuns(limitRaw));
    });
    return;
  }

  if (method === 'POST' && pathname === '/api/ops/missions/runs/clear') {
  ...
  if (method === 'GET' && pathname === '/api/ops/missions/policy') {
    await withMetric('ops-missions-policy-get', () => {
      sendJson(response, 200, {
        ok: true,
        policy: currentOpsMissionPolicyState(),
      });
  ...
  if (method === 'POST' && pathname === '/api/ops/missions/policy') {
    await withMetric('ops-missions-policy-update', async () => {
      const body = await readJsonBody(request);
      sendJson(response, 200, {
        ok: true,
        policy: updateOpsMissionPolicyState(body),
      });
    });
    return;
  }

  if (method === 'POST' && pathname === '/api/ops/missions/policy/suspend') {
    await withMetric('ops-missions-policy-suspend', async () => {
      const body = await readJsonBody(request);
      ...
      sendJson(response, 200, {
        ok: true,
        policy: suspendOpsMissionPolicyState(reason),
      });
    });
    return;
  }

  if (method === 'POST' && pathname === '/api/ops/missions/policy/resume') {
    await withMetric('ops-missions-policy-resume', async () => {
      const body = await readJsonBody(request);
      const reason = typeof body.reason === 'string' && body.reason.trim().length > 0
        ? body.reason.trim()
        : 'manual-resume';
      sendJson(response, 200, {
        ok: true,
        policy: resumeOpsMissionPolicyState(reason),
      });
    });
    return;
  }
  ...
  if (method === 'GET' && pathname === '/api/ops/missions/schedules/forecast') {
    await withMetric('ops-missions-schedules-forecast', () => {
      const horizonMsRaw = Number(url.searchParams.get('horizonMs') ?? '1200000');
      sendJson(response, 200, missionSchedulePolicyForecast(horizonMsRaw));
    });
    return;
  }
  ...
  if (method === 'POST' && pathname === '/api/ops/missions/schedules/resume-all') {
    await withMetric('ops-missions-schedules-resume-all', () => {
      sendJson(response, 200, setAllOpsMissionSchedulesActive(true));
    });
    return;
  }

  if (method === 'POST' && pathname === '/api/ops/missions/schedules/rebalance') {
  ...
  if (method === 'DELETE' && pathname.startsWith('/api/ops/missions/')) {
    await withMetric('ops-missions-delete', () => {
      const name = decodeURIComponent(pathname.slice('/api/ops/missions/'.length));
      const ok = deleteOpsMission(name);
      if (!ok) {
        sendJson(response, 404, { error: `mission '${name}' not found` });
        return;
      }
      sendJson(response, 200, { ok: true, name });
    });
...
https://developer.android.com</textarea>
  ...
  {"name":"recover-lane"}
          ...
          <hr style="border:0;border-top:1px solid #2f4a61;" />
          <input id="ops-mission-name" placeholder="mission name" value="release-guard-default" />
          <div class="split">
            <input id="ops-mission-scenario" placeholder="scenario name" value="stability-default" />
            <input id="ops-mission-policy-preset" placeholder="policy preset" value="balanced-default" />
          </div>
          <div class="split">
            <input id="ops-mission-watchdog-profile" placeholder="watchdog profile" value="watchdog-default" />
            <select id="ops-mission-auto-run-queue">
              <option value="true">auto-run queue: true</option>
              <option value="false">auto-run queue: false</option>
            </select>
          </div>
          <textarea id="ops-mission-queue-plan">[
  {"type":"scenario-run","payload":{"name":"stability-default"}},
  {"type":"watchdog","payload":{"autoHealOnAlert":true}}
]</textarea>
          <div class="split">
            <button class="s" id="ops-missions-list-btn">List missions</button>
            <button class="s" id="ops-mission-save-btn">Save mission</button>
          </div>
          <div class="split">
            <button class="p" id="ops-mission-plan-btn">Plan mission</button>
            <button class="p" id="ops-mission-run-btn">Run mission</button>
          </div>
          <div class="split">
            <button class="s" id="ops-mission-runs-btn">Load mission runs</button>
            <button class="w" id="ops-mission-runs-clear-btn">Clear mission runs</button>
          </div>
          ...
          <div class="split">
            <button class="w" id="ops-mission-schedules-pause-all-btn">Pause all schedules</button>
            <button class="p" id="ops-mission-schedules-resume-all-btn">Resume all schedules</button>
          </div>
          <div class="split">
            <input id="ops-mission-schedules-factor" type="number" step="0.1" min="0.1" value="1.2" />
            <button class="s" id="ops-mission-schedules-rebalance-btn">Rebalance schedules</button>
          </div>
          <div class="split">
            <input id="ops-mission-forecast-horizon-ms" type="number" min="60000" value="1200000" />
            <button class="s" id="ops-mission-forecast-btn">Forecast schedule runs</button>
          </div>
          ...
          <div class="split">
            <select id="ops-mission-policy-block">
              <option value="true">block on gate fail: true</option>
              <option value="false">block on gate fail: false</option>
            </select>
            <input id="ops-mission-policy-max-failures" type="number" min="1" value="3" />
          </div>
          <div class="split">
            <input id="ops-mission-policy-cooldown-ms" type="number" min="5000" value="180000" />
            <select id="ops-mission-policy-auto-resume">
              <option value="true">auto-resume: true</option>
              <option value="false">auto-resume: false</option>
            </select>
          </div>
          <div class="split">
            <button class="s" id="ops-mission-policy-load-btn">Load mission policy</button>
            <button class="s" id="ops-mission-policy-save-btn">Save mission policy</button>
          </div>
          <div class="split">
            <button class="w" id="ops-mission-policy-suspend-btn">Suspend scheduler</button>
            <button class="p" id="ops-mission-policy-resume-btn">Resume scheduler</button>
          </div>
          <div id="ops-mission-policy-panel" class="metrics"></div>
        </article>
    ...
    <script>
      ...
      const $opsMissionName = document.getElementById('ops-mission-name');
      const $opsMissionScenario = document.getElementById('ops-mission-scenario');
      const $opsMissionPolicyPreset = document.getElementById('ops-mission-policy-preset');
      const $opsMissionWatchdogProfile = document.getElementById('ops-mission-watchdog-profile');
      const $opsMissionAutoRunQueue = document.getElementById('ops-mission-auto-run-queue');
      ...
      const $opsMissionPolicyAutoResume = document.getElementById('ops-mission-policy-auto-resume');
      ...
      function renderOpsMissions(payload) {
        if (!$opsMissionsPanel) {
          return;
        }
        const missions = Array.isArray(payload && payload.missions) ? payload.missions : [];
        $opsMissionsPanel.innerHTML = '';
        for (const mission of missions.slice(0, 25)) {
          const row = document.createElement('div');
          row.className = 'item';
          row.innerHTML =
            '<div class="meta">' + mission.name + '</div>' +
            '<div>scenario=' + (mission.scenarioName || '-') + ' policyPreset=' + (mission.policyPresetName || '-') + '</div>' +
            '<div>watchdog=' + (mission.watchdogProfileName || '-') + ' queuePlan=' + ((mission.queuePlan && mission.queuePlan.length) || 0) + ' autoRun=' + String(mission.autoRunQueue !== false) + '</div>';
          $opsMissionsPanel.appendChild(row);
        }
        if (!missions.length) {
          $opsMissionsPanel.textContent = 'No missions.';
        }
      }

      function renderOpsMissionRuns(payload) {
        if (!$opsMissionRunsPanel) {
          return;
        }
        const runs = Array.isArray(payload && payload.runs) ? payload.runs : [];
        $opsMissionRunsPanel.innerHTML = '';
        for (const run of runs.slice(0, 30)) {
          const row = document.createElement('div');
          row.className = 'item';
          row.innerHTML =
            '<div class="meta">#' + run.id + ' ' + run.mission + ' [' + (run.ok ? 'ok' : 'fail') + ']</div>' +
            '<div>started=' + run.startedAt + ' durationMs=' + (run.durationMs || 0) + '</div>' +
            '<div>steps=' + ((run.steps && run.steps.length) || 0) + ' gate=' + (run.gate && run.gate.pass === true ? 'pass' : 'fail') + '</div>' +
            (run.error ? '<div style="color:#ff8f8f;">' + run.error + '</div>' : '');
          $opsMissionRunsPanel.appendChild(row);
        }
        if (!runs.length) {
      ...
      function renderOpsMissionForecast(payload) {
        if (!$opsMissionForecastPanel) {
          return;
        }
        const items = Array.isArray(payload && payload.items) ? payload.items : [];
        $opsMissionForecastPanel.innerHTML = '';
        const head = document.createElement('div');
        head.className = 'item';
        head.innerHTML =
          '<div class="meta">forecast horizonMs=' + (payload.horizonMs || 0) + '</div>' +
          '<div>suspended=' + String(payload.suspended === true) + ' resumeAt=' + (payload.resumeAt || '-') + '</div>' +
          '<div>plannedRuns=' + items.length + '</div>';
        $opsMissionForecastPanel.appendChild(head);
        for (const item of items.slice(0, 30)) {
          const row = document.createElement('div');
          row.className = 'item';
          row.innerHTML =
            '<div class="meta">[' + item.at + '] #' + item.id + ' ' + item.name + '</div>' +
            '<div>mission=' + item.mission + '</div>';
          $opsMissionForecastPanel.appendChild(row);
        }
      }

      function renderOpsMissionPolicy(payload) {
        if (!$opsMissionPolicyPanel) {
          return;
        }
        const policy = payload && payload.policy ? payload.policy : payload || {};
        $opsMissionPolicyPanel.innerHTML = '';
        const row = document.createElement('div');
        row.className = 'item';
        row.innerHTML =
          '<div class="meta">scheduler policy</div>' +
          '<div>block=' + String(policy.blockSchedulesOnGateFail !== false) + ' maxFailures=' + (policy.maxConsecutiveFailures || 0) + '</div>' +
          '<div>cooldownMs=' + (policy.cooldownMs || 0) + ' autoResume=' + String(policy.autoResumeAfterCooldown !== false) + '</div>' +
      ...
      async function listOpsMissionsUi() {
        const result = await api('/api/ops/missions');
        renderOpsMissions(result);
        renderOutput(result);
        const missions = Array.isArray(result.missions) ? result.missions : [];
        if (missions.length > 0) {
          const top = missions[0];
          $opsMissionName.value = top.name || '';
          $opsMissionScenario.value = top.scenarioName || '';
          $opsMissionPolicyPreset.value = top.policyPresetName || '';
          $opsMissionWatchdogProfile.value = top.watchdogProfileName || '';
          $opsMissionAutoRunQueue.value = top.autoRunQueue === false ? 'false' : 'true';
          $opsMissionQueuePlan.value = JSON.stringify(top.queuePlan || [], null, 2);
        }
        setMessage('Ops missions loaded', false);
      }

      async function saveOpsMissionUi() {
        const name = ($opsMissionName.value || '').trim();
        if (!name) {
          throw new Error('mission name required');
        }
        let queuePlan = [];
        try {
          const parsed = JSON.parse($opsMissionQueuePlan.value || '[]');
          queuePlan = Array.isArray(parsed) ? parsed : [];
        } catch (error) {
          throw new Error('mission queue plan must be valid JSON array');
        }
        const result = await api('/api/ops/missions', 'POST', {
          name,
          description: 'web-ui mission',
          scenarioName: ($opsMissionScenario.value || '').trim() || undefined,
          policyPresetName: ($opsMissionPolicyPreset.value || '').trim() || undefined,
          watchdogProfileName: ($opsMissionWatchdogProfile.value || '').trim() || undefined,
          preflightUrl: ($opsScenarioUrl.value || '').trim() || undefined,
          autoRunQueue: asBoolean($opsMissionAutoRunQueue.value, true),
          queuePlan,
          gatePolicy: {
            minScore: Number($opsGateMinScore.value || '80'),
            allowedSeverity: $opsGateSeverity.value || 'green',
        ...
        renderOutput(result);
        setMessage('Ops mission saved', false);
        await listOpsMissionsUi();
      }

      async function planOpsMissionUi() {
        const name = ($opsMissionName.value || '').trim();
        if (!name) {
          throw new Error('mission name required');
        }
        const result = await api('/api/ops/missions/plan', 'POST', { name });
        renderOutput(result);
        if (result.gatePreview) {
          renderGate(result.gatePreview);
        }
        setMessage(result.ready ? 'Mission plan ready' : 'Mission plan has warnings', !result.ready);
      }

      async function runOpsMissionUi() {
        const name = ($opsMissionName.value || '').trim();
        if (!name) {
          throw new Error('mission name required');
        }
        const result = await api('/api/ops/missions/run', 'POST', {
          name,
          laneId: selectedLaneId(),
          deviceId: selectedDeviceId(),
          url: ($opsScenarioUrl.value || '').trim() || undefined,
        ...
        setMessage(result.ok ? 'Mission executed' : 'Mission execution failed', !result.ok);
        await listOpsMissionRunsUi();
        await refreshJobsAndLanes();
      }

      async function listOpsMissionRunsUi() {
        const result = await api('/api/ops/missions/runs?limit=120');
        renderOpsMissionRuns(result);
        renderOutput(result);
        setMessage('Mission runs loaded', false);
      }

      async function clearOpsMissionRunsUi() {
        const result = await api('/api/ops/missions/runs/clear', 'POST', {
          keepLatest: 20,
        });
        renderOutput(result);
        setMessage('Mission runs cleared', false);
        await listOpsMissionRunsUi();
      }
      ...
      async function loadOpsMissionPolicyUi() {
        const result = await api('/api/ops/missions/policy');
        renderOpsMissionPolicy(result);
        renderOutput(result);
        const policy = result.policy || {};
        $opsMissionPolicyBlock.value = policy.blockSchedulesOnGateFail === false ? 'false' : 'true';
        $opsMissionPolicyMaxFailures.value = String(policy.maxConsecutiveFailures || 3);
        $opsMissionPolicyCooldownMs.value = String(policy.cooldownMs || 180000);
        $opsMissionPolicyAutoResume.value = policy.autoResumeAfterCooldown === false ? 'false' : 'true';
        setMessage('Mission policy loaded', false);
      }

      async function saveOpsMissionPolicyUi() {
        const result = await api('/api/ops/missions/policy', 'POST', {
          blockSchedulesOnGateFail: asBoolean($opsMissionPolicyBlock.value, true),
          maxConsecutiveFailures: Number($opsMissionPolicyMaxFailures.value || '3'),
          cooldownMs: Number($opsMissionPolicyCooldownMs.value || '180000'),
          autoResumeAfterCooldown: asBoolean($opsMissionPolicyAutoResume.value, true),
        });
        renderOpsMissionPolicy(result);
        renderOutput(result);
        setMessage('Mission policy updated', false);
      }

      async function suspendOpsMissionPolicyUi() {
        const result = await api('/api/ops/missions/policy/suspend', 'POST', {
          reason: 'web-ui-manual-suspend',
        });
        renderOpsMissionPolicy(result);
        renderOutput(result);
        setMessage('Mission scheduler suspended', false);
      }

      async function resumeOpsMissionPolicyUi() {
        const result = await api('/api/ops/missions/policy/resume', 'POST', {
          reason: 'web-ui-manual-resume',
        });
        renderOpsMissionPolicy(result);
        renderOutput(result);
        setMessage('Mission scheduler resumed', false);
      }

      async function pauseAllOpsMissionSchedulesUi() {
        const result = await api('/api/ops/missions/schedules/pause-all', 'POST', {});
        renderOpsMissionSchedules(result);
        renderOutput(result);
        setMessage('All mission schedules paused', false);
      }

      async function resumeAllOpsMissionSchedulesUi() {
        const result = await api('/api/ops/missions/schedules/resume-all', 'POST', {});
        renderOpsMissionSchedules(result);
        renderOutput(result);
        setMessage('All mission schedules resumed', false);
      }

      async function rebalanceOpsMissionSchedulesUi() {
        const result = await api('/api/ops/missions/schedules/rebalance', 'POST', {
          factor: Number($opsMissionSchedulesFactor.value || '1'),
          minEveryMs: 5000,
          maxEveryMs: 24 * 60 * 60 * 1000,
        });
        renderOpsMissionSchedules(result);
        renderOutput(result);
        setMessage('Mission schedules rebalanced', false);
      }

      async function forecastOpsMissionSchedulesUi() {
        const horizonMs = Number($opsMissionForecastHorizonMs.value || '1200000');
        const result = await api('/api/ops/missions/schedules/forecast?horizonMs=' + encodeURIComponent(String(horizonMs)));
        renderOpsMissionForecast(result);
        renderOutput(result);
      ...
      document.getElementById('ops-missions-list-btn').addEventListener('click', async function () {
        try { await listOpsMissionsUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-save-btn').addEventListener('click', async function () {
        try { await saveOpsMissionUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-plan-btn').addEventListener('click', async function () {
        try { await planOpsMissionUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-run-btn').addEventListener('click', async function () {
        try { await runOpsMissionUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-runs-btn').addEventListener('click', async function () {
        try { await listOpsMissionRunsUi(); } catch (error) { setMessage(String(error), true); }
      });
      ...
      document.getElementById('ops-mission-schedules-pause-all-btn').addEventListener('click', async function () {
        try { await pauseAllOpsMissionSchedulesUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-schedules-resume-all-btn').addEventListener('click', async function () {
        try { await resumeAllOpsMissionSchedulesUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-schedules-rebalance-btn').addEventListener('click', async function () {
        try { await rebalanceOpsMissionSchedulesUi(); } catch (error) { setMessage(String(error), true); }
      });
      ...
      document.getElementById('ops-mission-policy-save-btn').addEventListener('click', async function () {
        try { await saveOpsMissionPolicyUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-policy-suspend-btn').addEventListener('click', async function () {
        try { await suspendOpsMissionPolicyUi(); } catch (error) { setMessage(String(error), true); }
      });
      document.getElementById('ops-mission-policy-resume-btn').addEventListener('click', async function () {
        try { await resumeOpsMissionPolicyUi(); } catch (error) { setMessage(String(error), true); }
      });
